...
async function ensureSchema() {
  ...
  if (!schemaPromise) {
    schemaPromise = (async () => {
      // Neon serverless compute auto-suspends when idle; the first connection after
      // idle can time out while it resumes. Retry with backoff so a cold start does
      // not 500 the request.
      let lastErr;
      ...
      // Don't cache a rejected promise (it would poison this lambda until recycle);
      // let the next request retry a fresh connection.
      schemaPromise = null;
      throw err;
    });
  }
